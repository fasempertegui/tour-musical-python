...
class VistaUbicacion(ctk.CTkFrame):
    def __init__(self, master=None, controlador=None):
        ...
        self.mapa.add_right_click_menu_command(label="Estoy aqui", command=self._anadir_marcador, pass_coords=True)
        ...
        boton_aceptar = ctk.CTkButton(frame_botones, text="Aceptar", command=self._guardar_cambios, **VistaUtils.estilo_boton)
        boton_aceptar.grid_configure(row=0, column=0, padx=5, pady=10)
        boton_aceptar.grid()

        boton_cancelar = ctk.CTkButton(frame_botones, text="Cancelar", command=self._regresar, **VistaUtils.estilo_boton)
        boton_cancelar.grid_configure(row=0, column=1, padx=5, pady=10)
        boton_cancelar.grid()

    # Privados

    def _anadir_marcador(self, coordenadas):
